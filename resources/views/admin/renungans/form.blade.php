...
@section('title', ($item->exists ? 'Edit' : 'Tambah') . ' Renungan')

@section('content')
<h4 class="mb-3">{{ $item->exists ? 'Edit' : 'Tambah' }} Renungan</h4>

<form method="POST"
      action="{{ $item->exists ? route('admin.renungans.update',$item) : route('admin.renungans.store') }}"
  ...
  <div class="col-lg-8">
    ...
    <div class="card shadow-sm">
      <div class="card-body d-flex gap-2">
        ...
        <a href="{{ route('admin.renungans.index') }}" class="btn btn-outline-secondary">Kembali</a>
      </div>
    </div>
  </div>
