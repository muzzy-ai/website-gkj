...
@section('title', ($item->exists ? 'Edit' : 'Tambah') . ' Berita')
...
<h4 class="mb-3">{{ $item->exists ? 'Edit' : 'Tambah' }} Berita</h4>

<form method="POST"
      action="{{ $item->exists ? route('admin.beritas.update',$item) : route('admin.beritas.store') }}"
  ...
  <div class="col-lg-8">
    <div class="card shadow-sm mb-3">
      <div class="card-body">
        ...
        <div class="mb-3">
          ...
          <div class="form-text">Digunakan untuk listing & meta description jika tersedia.</div>
        </div>

        <div class="mb-3">
          <label class="form-label">Isi Berita</label>
          <textarea name="body" rows="12" class="form-control" required>{{ old('body',$item->body) }}</textarea>
        </div>
        ...
        <div class="row">
          ...
          <div class="col-md-6 mb-3">
            ...
            <input type="text" name="tags" class="form-control" value="{{ old('tags',$item->tags) }}" placeholder="Pisahkan dengan koma, mis: pemuda,diakonia">
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="card shadow-sm">
      <div class="card-body d-flex gap-2">
        ...
        <a href="{{ route('admin.beritas.index') }}" class="btn btn-outline-secondary">Kembali</a>
      </div>
    </div>
  </div>
    ...
    <div class="card shadow-sm">
      <div class="card-body">
        ...
        <div class="mb-0">
          ...
          <textarea name="meta_description" rows="3" class="form-control">{{ old('meta_description',$item->meta_description ?? $item->excerpt) }}</textarea>
        </div>
      </div>
    </div>
  </div>
